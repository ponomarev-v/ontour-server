...
//смс?
    ...
    public static function FindSmth($TabName, $ColName, $query){
        ...
        if (isset($TabName) && !empty($TabName) && isset($ColName) && !empty($ColName) && isset($query) && !empty($query)){
            $query = $query.'%'; 
            $res   = $db -> rawQuery("SELECT count(*) FROM $TabName WHERE $ColName LIKE '$query'");
            return $res;
        } else {
    ...
    public static function newsendSMS($phone, $msg){
        ...
        $smsru = new SMSRU('3F7F91FE-ED9B-F4F7-AB4A-5B770FB08A4F'); // Ваш уникальный программный ключ, который можно получить на главной странице
